...
import { views, initMap, sumData, diffFunc, type DiffType, type ICityData } from '@/composables/taiwan'
...
const years = Array.from({ length: 13 }).map((y, i) => i + 2010)
const cntYear = ref(0)
...
const barChartData = computed(() => {
  ...
  for (const c in sumData) {
    ...
    for (const y in _city) {
      const negative = (cntView.value !== 'POP' && cntViewIndex.value === 1 && -1) || 1
      const _v = { code: c, value: diffFunc(cntView.value, _city[y])[cntViewIndex.value] * negative }
...
const dynamicRange = computed(() => views[cntView.value].range[cntViewIndex.value])
const colorFunc = d3.scaleLinear([min, 0, max], ['blue', 'white', 'orange'])

onMounted(() => {
  const { updateColor, selectCity: mapCity } = initMap((obj) => (cntCityData.value = obj))
  const { updateChart, selectCity: chartCity } = initChart((obj) => (cntCityData.value = obj))
...
<template>
  <main class="h-full flex overflow-hidden">
    <MapLayout>
      <template #main>
        <div class="overflow-scroll flex flex-col gap-4">
          <div class="w-full flex items-center">
            <h1 class="text-[28px] font-bold">
              <span class="w-20 inline-block text-center"> {{ years[cntYear] }}</span> 人口變化圖
            </h1>

            <select v-model="cntView" class="ml-auto border border-gray-300 p-2 rounded-lg">
        ...
        <div class="absolute right-8 bottom-8">
          <Suspense v-if="renderCom && cntCityData">
            <Chart
              :title="`${cntCityData.name} ${views[cntView].title}`"
              :file="cityData!"
              :index="3"
              :cnt-year="years[cntYear]"
      ...
      <template #aside>
        <div class="flex flex-col gap-4 overflow-scroll">
          <div class="border w-[400px] border-gray-300 h-max rounded-lg p-4 bg-white">
