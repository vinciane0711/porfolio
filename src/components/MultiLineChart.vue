...
import { conf, initChart, csvConvertor, type ICsvObj } from '@/composables/taiwan/multiLine'
...
const data = typeof props.file === 'string' ? await csvConvertor(props.file) : props.file

const { keys, years, rest, rates, line, line2, area, transposeFunc, drawDetail } = initChart(data, props.index)
  ...
  <div class="p-4 rounded-lg flex gap-4 flex-col w-[400px] border transition bg-white border-gray-300">
    ...
    <svg ref="el" :viewBox="`0, 0, ${conf.w}, ${conf.h}`">
      ...
      <g class="multi-lines" stroke-width="2">
        <path v-for="(r, i) in rest" fill="none" :stroke="palette[i]" :d="`${line(transposeFunc(r))}`" />
        <path fill="none" :stroke="palette[2]" stroke-dasharray="4 3" :d="`${line2(transposeFunc(rates))}`" />
        <path v-if="!colors" v-for="(r, i) in rest" :fill="`url(#grad${[i + 1]})`" :d="`${area(transposeFunc(r))}`" />
      </g>
      ...
      <g class="focus" opacity="0.75" :transform="`translate(${conf.mx},0)`">
        <line
          ...
          :y2="conf.mt"
        />
        <circle v-for="(l, i) in keys" r="4" :fill="palette[i]" stroke="white" stroke-width="1.5" />
      </g>
    </svg>
    ...
    <div class="grid grid-cols-3 gap-2 text-sm px-10">
      <div v-for="(l, i) in keys">
        <div class="flex items-center justify-center text-xs">
          <span class="w-2 h-2 rounded-full mr-2" :style="{ background: palette[i] }" />
          <p>{{ l }}</p>
        </div>
        <p class="text-center font-bold">
          {{ rest[i] ? numWithCommas(rest[i][yearIndex]) : numWithCommas(rates[yearIndex]) }}
        </p>
      </div>
    </div>
  </div>
